...
fn fuzz_like_random_ops() {
    ...
    let cfg = Config { path: tmp.path().to_path_buf(), index_capacity: 512, free_capacity: 512, initial_file_size: 8 * 1024 * 1024 };
            ...
            match op {
                ...
                1 => { // remove
                    ...
                    if let (Some(v1), Some(v2)) = (r1, r2) {
                        if v1 != v2 {
                            eprintln!("WARNING: fuzz mismatch on key {}: {} vs {}", key, v1.len(), v2.len());
                        }
                    }
                }
                2 => { // get
    ...
    if res.is_err() {
        eprintln!("Fuzz harness panicked; temp file is at: {}", tmp.path().display());
